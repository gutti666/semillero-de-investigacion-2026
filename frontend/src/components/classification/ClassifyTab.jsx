  ...
  return (
    <section>
      ...
      <div className="flex flex-wrap items-end gap-5 mt-5">
        <RangeSlider id="conf-cls" label="Confianza" value={conf} onChange={setConf} />
        <RangeSlider id="iou-cls"  label="IoU"       value={iou}  onChange={setIou} />
        <RangeSlider id="imgsz-cls" label="Img size" value={imgsz} onChange={setImgsz} min={32} max={1280} step={32} />
        ...
        <button
          ...
          "
        >
          {isPending ? 'Clasificando…' : 'Clasificar Alga'}
        </button>
      </div>

      <Spinner visible={isPending} />
      <ErrorBox message={errMsg} />
      <ClassifyResult data={data} />
    </section>
  )
}
